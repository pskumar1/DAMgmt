--Provide the FULL active organization hierarchy for Vice President - Operations [Employee Id: E1003]. Include the Person Addresses as well.
WITH RECURSIVE HIERARCHY (PERSON_ID, EMPLID, FIRST_NAME, LAST_NAME, JOB_TITLE, STATUS_TYP) AS 
 (
	SELECT PERSON_ID, EMPLID,FIRST_NAME, LAST_NAME, JOB_TITLE,STATUS_TYP  FROM HR.PERSON WHERE EMPLID = 'E1003' -- NON RECURSIVE TERM
	UNION ALL
	SELECT A.PERSON_ID, A.EMPLID, A.FIRST_NAME, A.LAST_NAME, A.JOB_TITLE,A.STATUS_TYP FROM HR.PERSON A INNER JOIN HIERARCHY B ON A.MANAGER_EMPLID = B.EMPLID   --RECURSIVE TERM
 )
